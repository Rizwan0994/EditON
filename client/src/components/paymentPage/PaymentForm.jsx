...
function PaymentForm() {
  ...
  const handleToken = async (token) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_NODE_API}api/payment`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="payment-page">
      <h1 className='heading'>Please Fill Out Required Fields</h1>
      <div className="product-info">
        <div className="product-name">
          <label htmlFor="creatorName">Creator name:</label>
          <input type="text" id="creatorName" value={creatorName} onChange={(e) => setCreatorName(e.target.value)} />
        </div>
        <br></br>
        <div className="product-email">
          <label htmlFor="creatorEmail">Creator Email:</label>
          <input  type="email" id="creatorEmail" value={creatorEmail} onChange={(e) => setCreatorEmail(e.target.value)} />
        </div>
        <br></br>
        <div className="product-price">
          <label htmlFor="creatorPrice">Creator Price:</label>
          <input type="number" id="creatorPrice" value={creatorPrice} onChange={(e) => setCreatorPrice(parseFloat(e.target.value))} />
        </div>
      </div>
      <button className='payment-page-button' onClick={handleAddOrder}>Click to Add</button>
      <br></br>
      <br></br>
