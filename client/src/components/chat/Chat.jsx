...
function Chat() {
  ...
  const handleSendFile = async () => {
    if (selectedFile) {
      try {
        const cloudinaryResponse = await cloudinary.v2.uploader.upload(selectedFile.path);
        const fileMessage = {
  ...
  return (
    <Container maxWidth="md">
      <h1 style={{marginTop:'20px', fontSize:"30px"}}>Chat Window</h1>
      <div style={{ minHeight: '300px', border: '1px solid #ccc', borderRadius: '5px', padding: '10px', marginBottom: '20px', overflowY: 'auto' }}>
        {messages.map((message, index) => (
          <Paper
            ...
            style={{
              ...
              backgroundColor: message.user.id === username.id ? '#4CAF50' : '#2196F3',
              color: 'white',
              borderRadius: '5px',
            }}
          >
      ...
      <Button variant="contained" color="primary" onClick={handleSendMessage} style={{ marginLeft: '10px', marginTop: '10px' }}>
        Send
      </Button>
      {/* <Input
        type="file"
